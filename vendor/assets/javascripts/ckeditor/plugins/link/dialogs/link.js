...
( function() {
  CKEDITOR.dialog.add( 'link', function( editor ) {
    ...
    // Handles the event when the "Type" data box is changed.
        ...
        },
        {
          ...
          children: [ {
            ...
            },
            {
              ...
              validate: function() {
                var dialog = this.getDialog();
                var art_id = this.getDialog().getContentElement( 'info', 'articleId' ).getValue();
  
                if ( (dialog.getContentElement( 'info', 'linkType' ) && dialog.getValueOf( 'info', 'linkType' ) != 'url') || art_id != "" ){
                  if (art_id != "" && $.isNumeric(art_id) ) {
                    return true;
                  }else{
                    alert(commonLang.invalidArticleID);
                    this.getDialog().getContentElement( 'info', 'articleId' ).focus();
                    return false;
                  };
                  return true;
                }
              ...
              setup: function( data ) {
                ...
                if ( data.url ) {
                  ...
                  var domain = url.substr(0,url.indexOf('/'));
                  var param = url.substr(url.indexOf('/')+1);

                  if ( domain == location.host && $.isNumeric(param) && data.url.protocol == 'http://' ) {
                    this.setValue( "" );
                  }else{
              ...
              commit: function( data ) {
                ...
                var protocol = this.getDialog().getContentElement( 'info', 'protocol' ).getValue();
                if ( art_id != "" ) {
                  if ( !data.url )
                    data.url = {};
                  data.url.articleId = this.getValue();
                  data.url.protocol = 'http://';
                  data.url.url = location.host + "/" + art_id;
                } else {
                  if ( !data.url )
                    data.url = {};
                  data.url.articleId = '';
                  data.url.protocol = protocol;
